...
def DFS_simple(g, u, t, discovered):
  """Perform DFS of the undiscovered portion of Graph g starting at Vertex u 
  and finishing at Vertex t.
  If t is found, it returns True, False otherwise
  ...
  for e in g.incident_edges(u):             # for every outgoing edge from u
    ...
    if v not in discovered:                 # v is an unvisited vertex
      discovered[v] = e                     # e is the tree edge that discovered v
      if(v == t):                           # if t is found, 
        return True                         # it returns True
      if(DFS_simple(g, v, t, discovered)):   # otherwise it recursively explore from v
        return True                         
  return False